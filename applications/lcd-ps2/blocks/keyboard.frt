...
variable kbd_FLGR     \ flags, final hi=|alt|ctrl|shift|num|releas|extend|0|0|
...
\ --- Receive bits --
  ...
  R17 kbd_ROTR 1+ lds,
  clc,                \ CY=0
  PORTB 22 - 1 sbic,  \ PinB.1=1 then CY=1
  sec,
 ...
 adr> brne,   0 >lbl  \ all 8+3 bits? else end
  ...
  R16 rol,
  R17 rol,            \ CY=stopbit
 adr> brcc,   1 >lbl  \ CY=0 then end
  R16 rol,
  R17 rol,            \ CY=parity, data
  R16 rol,            \ CY=startbit
 adr> brcs,   2 >lbl  \ CY=1? then end
